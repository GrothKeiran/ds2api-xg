...
func TestStartCompletionAppliesCurrentInputFileGlobally(t *testing.T) {
	...
	if got := ds.uploads[0].Filename; got != "conversation_history.txt" {
		t.Fatalf("upload filename=%q want conversation_history.txt", got)
	}
	...
	if !start.Request.CurrentInputFileApplied || !strings.Contains(start.Request.PromptTokenText, "# conversation_history.txt") {
