...
func currentInputFilePrompt(hasToolsFile bool) string {
	prompt := "Continue from the latest state in the attached conversation_history.txt context. Treat it as the current working state and answer the latest user request directly."
	if hasToolsFile {
		prompt += " Available tool descriptions and parameter schemas are attached in tool_definitions.txt; use only those tools and follow the tool-call format rules in this prompt."
	}
	return prompt
}
