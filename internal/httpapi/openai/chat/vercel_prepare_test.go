...
func TestHandleVercelStreamSwitchReuploadsCurrentInputFile(t *testing.T) {
	...
	stdReq := promptcompat.StandardRequest{
		...
		FinalPrompt:             "Continue from the latest state in the attached conversation_history.txt context. Available tool descriptions and parameter schemas are attached in tool_definitions.txt; use only those tools and follow the tool-call format rules in this prompt.",
		PromptTokenText:         "# conversation_history.txt\n\n=== 1. USER ===\nhello\n\n# tool_definitions.txt\nAvailable tool descriptions and parameter schemas for this request.\n\nYou have access to these tools:\n\nTool: search\nDescription: search docs\nParameters: {\"type\":\"object\"}\n",
		HistoryText:             "# conversation_history.txt\n\n=== 1. USER ===\nhello\n",
	...
	promptText, _ := payload["prompt"].(string)
	if !strings.Contains(promptText, "tool_definitions.txt") {
