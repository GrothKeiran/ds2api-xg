...
func TestChatCompletionsCurrentInputFilePersistsNeutralPrompt(t *testing.T) {
	...
	if ds.uploadCalls[0].Filename != "conversation_history.txt" {
		t.Fatalf("expected conversation_history.txt upload, got %q", ds.uploadCalls[0].Filename)
	}
	...
	if !strings.Contains(full.Messages[0].Content, "Continue from the latest state in the attached conversation_history.txt context.") {
