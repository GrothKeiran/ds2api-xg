...
func TestClaudeDirectAppliesCurrentInputFile(t *testing.T) {
	...
	if ds.uploads[0].Filename != "conversation_history.txt" {
	...
	if !strings.Contains(prompt, "Continue from the latest state in the attached conversation_history.txt context.") {
	...
	if len(full.Messages) != 1 || !strings.Contains(full.Messages[0].Content, "Continue from the latest state in the attached conversation_history.txt context.") {
...
func TestClaudeCurrentInputFileUploadsToolsSeparately(t *testing.T) {
	...
	if ds.uploads[0].Filename != "conversation_history.txt" || ds.uploads[1].Filename != "tool_definitions.txt" {
