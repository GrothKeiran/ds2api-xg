...
func TestBuildOpenAIPromptWithToolInstructionsOnlyOmitsSchemas(t *testing.T) {
	...
	if !strings.Contains(finalPrompt, "Treat tool_definitions.txt as the authoritative list of callable tools and schemas") {
...
func TestBuildOpenAIToolsContextTranscriptContainsOnlyDescriptions(t *testing.T) {
	...
	for _, want := range []string{"# tool_definitions.txt", "You have access to these tools", "Tool: search", "Description: search docs", `Parameters: {"type":"object"}`} {
