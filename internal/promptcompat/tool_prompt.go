...
const CurrentToolsContextFilename = "tool_definitions.txt"

const toolsTranscriptTitle = "# tool_definitions.txt"
...
func injectToolPromptWithDescriptions(messages []map[string]any, tools []any, policy ToolChoicePolicy, includeDescriptions bool) ([]map[string]any, []string) {
	...
	if includeDescriptions && parts.Descriptions != "" {
		...
		toolPrompt = "Available tool descriptions and parameter schemas are attached in tool_definitions.txt. Treat tool_definitions.txt as the authoritative list of callable tools and schemas; use only tools and parameters listed there.\n\n" + toolPrompt
	}
